...
class Images extends React.Component {
  ...
  render() {
    return (
      <div style={{
        background: "white",
        opacity: `${this.props.addtobag ? "0.2" : "1"}`,
        ...
        <section style={{boxSizing: "border-box", width: "55.3333333333%", float: "right", paddingLeft: "10px", paddingRight: "10px", paddingTop: "10px", position: "relative", display: "block"}}>
          <div className="DPmedia_player_container">
            {this.state.icons[0].toggle ? (
              <img className="DPmedia_zoom" src={this.state.current}/>
            ) : (this.state.image !== this.state.image2 ? (
              <div>
                <img className="DPmedia_main" src={this.state.image} style={{
                  position: "relative",
                  transition: "1s",
                  opacity: `${this.state.transition ? "0" : "1"}`
                }}/>
                <img className="DPmedia_main" src={this.state.image2} style={{
                  position: "absolute",
                  overflow: "hidden",
                  transition: "1s",
                  opacity: `${this.state.transition ? "1" : "0"}`,
                  width: "100%",
                  height: "93%"
                }}/>
              </div>
            ...
            <div className="DPside_media_container">
              {this.props.images.length <= 5 ? (
                      ...
                      <img className="DParrow" src='./Miscellaneous/icons8-collapse-arrow-30.png' onClick={this.changeUpView} style={{
                        opacity: `${this.state.translateValue === 0 ? "0.5" : "1" }`, 
                        cursor: `${this.state.translateValue === 0 ? "default" : "pointer"}`
                    }}/>)}
                <div className="DPside_media" >
                  {this.props.images.map((image, index) => (
                    <div className="DPside_single_container" key={index} onClick={() => this.updateMedia(image, index)} style={{
                      borderStyle: "solid",
                      borderWidth: `${this.state.side === index ? '1px' : '0px'}`,
                      borderColor: `${this.state.side === index ? 'black' : 'white'}`,
                ...
                {this.props.images.length <= 5 ? (
                    ...
                    <img className="DParrow" src='./Miscellaneous/icons8-expand-arrow-30.png' onClick={this.changeDownView} style={{
                      opacity: `${this.state.translateValue === (-117 * (this.props.images.length - 5)) ? "0.5" : "1" }`,
                      cursor: `${this.state.translateValue === (-117 * (this.props.images.length - 5)) ? 'default' : 'pointer'}`
                  }}/>)}
              </div>
        </section>
      </div>
